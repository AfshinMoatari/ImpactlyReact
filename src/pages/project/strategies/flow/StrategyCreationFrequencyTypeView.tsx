...
import { CalendarUnit, EndType, defaultFrequency } from "../../../../models/cron/Frequency";
...
interface StrategyCreationFrequencyTypeProp {
...
const StrategyCreationFrequencyTypeView: React.FC<StrategyCreationFrequencyTypeProp> = ({
    ...
    return (
        <Formik<SurveyFrequencyFormValues>
            initialValues={{
                expression: SendoutfrequencyToFrequencyExpression(defaultFrequency),
                end: defaultFrequency.end,
            } as SurveyFrequencyFormValues}
        ...
        >
            {formik => {
                ...
                const handleReset = async () => {
                    ...
                    setsurveyFrequencyFormValues({
                        ...surveyFrequencyFormValues,
                        expression: copyOfSurveyFrequencyFormValues.expression
                    });
                    ...
                    <Form
                        ...
                        <Grid
                            ...
                            <Grid item justifyContent="flex-start" xs={12}>
                                <p>{t("StrategyFlowPage.StrategyCreationFrequencyTypeView.chosenFrequency")}</p>
                            </Grid>

                            <Grid container item direction="column" justifyContent="flex-start" xs={12} style={{ gap: 10 }}>
                                <Grid item>
                                    <span style={{ fontWeight: 600 }}>
                                        {t("StrategyFlowPage.StrategyCreationFrequencyTypeView.sendEvery")}
                                    </span>
                                </Grid>
                                <Grid container item direction="row" style={{
                                    ...
                                    <Grid container item direction="row" style={{
                                        ...
                                        <Box style={{
                                            ...
                                            >
                                                <MenuItem value={CalendarUnit.DAILY}>
                                                    {t("StrategyFlowPage.StrategyCreationFrequencyTypeView.day")}
                                                </MenuItem>
                                                <MenuItem value={CalendarUnit.WEEKLY}>
                                                    {t("StrategyFlowPage.StrategyCreationFrequencyTypeView.week")}
                                                </MenuItem>
                                                <MenuItem value={CalendarUnit.MONTHLY}>
                                                    {t("StrategyFlowPage.StrategyCreationFrequencyTypeView.month")}
                                                </MenuItem>
                                            </Field>
                                        </Box>
                                    </Grid>
                                    <Box>
                                        <span className={classes.err}>{formik.errors.expression?.count?.toString()}</span>
                                    </Box>
                                </Grid>
                            </Grid>
                            ...
                            <Grid item direction="row" justifyContent="center" xs={12} style={{ gap: 10, marginTop: 20 }}>
                                {expression.unit === CalendarUnit.WEEKLY && (
                                    <Box pb={2}>
                                        ...
                                        <Box>
                                            <span className={classes.err}>{formik.errors.expression?.weekDays?.toString()}</span>
                                        </Box>
                                    </Box>
                                )}
                                ...
                                {expression.unit === CalendarUnit.MONTHLY && (
                                    <Box pb={2}>
                                        ...
                                        <Box>
                                            <span className={classes.err}>{formik.errors.expression?.monthDays?.toString()}</span>
                                        </Box>
                                    </Box>
                                )}
                            </Grid>
                            ...
                            <Grid container item direction="column" justifyContent="flex-start" xs={12} style={{ gap: 10 }}>
                                <Grid item>
                                    <span style={{ fontWeight: 600 }}>
                                        {t("StrategyFlowPage.StrategyCreationFrequencyTypeView.time")}
                                    </span>
                                </Grid>
                                <Grid container item direction="column">
                                    ...
                                    <Box>
                                        <span className={classes.err}>{formik.errors.expression?.hour?.toString()}</span>
                                        <span className={classes.err}>{formik.errors.expression?.minute?.toString()}</span>
                                    </Box>

                                </Grid>
                            </Grid>

                            <Grid container item direction="column" justifyContent="flex-start" xs={12} style={{ gap: 10, marginTop: 20 }}>
                                <Grid item>
                                    <span style={{ fontWeight: 600 }}>
                                        {t("StrategyFlowPage.StrategyCreationFrequencyTypeView.endAfter")}
                                    </span>
                                </Grid>
                                <Grid container item direction="row" style={{
                                    justifyContent: 'center',
                                    ...
                                    <Box>
                                        <Field
                                            ...
                                            onClick={() => toggleEnableReset(false)}
                                        >
                                            <MenuItem value={EndType.NEVER}>
                                                {t("StrategyFlowPage.StrategyCreationFrequencyTypeView.whenCompleted")}
                                            </MenuItem>
                                            <MenuItem value={EndType.OCCUR}>
                                                {t("StrategyFlowPage.StrategyCreationFrequencyTypeView.numberSendouts")}
                                            </MenuItem>
                                        </Field>
                                    </Box>
                                    <Box style={{
                                        ...
                                        <Field
                                            label={t("StrategyFlowPage.StrategyCreationFrequencyTypeView.sendouts")}
                                            as={TextField}
                                        ...
                                        />
                                        <span className={classes.err}>{EndType.OCCUR === end.type ? formik.errors.end?.occurrences?.toString() : null}</span>
                                    </Box>
                                </Grid>
                            </Grid>
                                        ...
                                        <Box>
                                            {edit && (
                                                <DirectionalButton
                                                    ...
                                                    onClick={handleReset}
                                                    text={t("StrategyFlowPage.StrategyCreationFrequencyTypeView.resetStep")}
                                                    variant="text"
                                                ></DirectionalButton>
                                            )}
                                        </Box>
                                    ...
                                    <Box style={{
                                        ...
                                        <Box>
                                            <DirectionalButton
                                                ...
                                                text={t("StrategyFlowPage.StrategyCreationFrequencyTypeView.back")}
                                                variant="outlined"
                                            ></DirectionalButton>
                                        </Box>
                                        <Box style={{
                                            ...
                                            <DirectionalButton
                                                type="submit"
                                                onClick={() => saveBatchSendoutData()}
                                                disabled={!isValid}
                                                text={t("StrategyFlowPage.StrategyCreationFrequencyTypeView.save")}
                                                aria-label="submit"
                                                variant="contained"
                                            >
                                            </DirectionalButton>
...
export default StrategyCreationFrequencyTypeView;
